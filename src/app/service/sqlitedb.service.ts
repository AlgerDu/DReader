...
export class SQLiteDbService {
    ...
    private initDb() {
        this.db.transaction(function (tx) {
            ...
            this.db.executeSql('CREATE TABLE BookShelf ( accUid char(36), bookUid char(36), readPct INT not null, updateCount INT not null, refreshTime DATETIME not null, lastReadVolumeNo INT not null, lastReadChapterIndex INT not null,PRIMARY KEY(accUid,bookUid))', []);
            this.db.executeSql('CREATE TABLE Book ( uid char(36) PRIMARY KEY, name TEXT not null, author TEXT not null, lastVolumeNo INT not null, lastChapterIndex INT not null, lastChapterName TEXT not null)', []);
            this.db.executeSql('CREATE TABLE Volume ( bookUid char(36), vNo INT, name TEXT not null, PRIMARY KEY(bookUid,vNo))', []);
            this.db.executeSql('CREATE TABLE Chapter ( uid char(36) PRIMARY KEY, bookUid char(36) not null, name TEXT not null, contentUid char(36), volumeNo INT not null, volumeIndex INT not null)', []);
